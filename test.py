...
def get_stat_value(row, field_code):
    ...
    if field_code in ["GS_percent", "GS%", "GS_pct"]:
        ...
        if not pd.isnull(gs) and not pd.isnull(games_played) and games_played >= 1:
            ...
            return np.nan
    # Pozostałe statystyki jak wcześniej
...
def load_data():
    ...
    return df_nba, df_wnba, nfl_sheets

df_nba, df_wnba, nfl_sheets = load_data()
...
# --------- WYBÓR LIGI ---------
liga = st.radio("Wybierz ligę do szczegółowej analizy:", ["NBA", "WNBA", "NFL"])
if liga == "NBA":
    df = df_nba
    fields = fields_nba
elif liga == "WNBA":
...
with tab2:
    ...
    with st.container():
        ...
        heatmap_stats = st.multiselect(
            ...
            key="heatmap_stats"   # ZAMIENIONE z "radar_stats" na "heatmap_stats"
        )
    ...
    if not pca_data.empty:
        ...
        n_clusters = min(3, len(pca_data)) if len(pca_data) >= 3 else 1
        if n_clusters > 1:
    ...
    if porownaj:
        ...
        # (poniższy kod jest taki jak miałeś, ale działa tylko po zaznaczeniu!)
